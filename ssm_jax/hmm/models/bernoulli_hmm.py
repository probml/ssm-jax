...
class BernoulliHMM(BaseHMM):
    ...
    def m_step(self, batch_emissions, batch_posteriors, **kwargs):
        ...
        self._transition_matrix = replace(self._initial_probabilities,
                                          value=tfd.Dirichlet(1.0001 + stats.trans_probs).mode())
        self._emission_probs = replace(self._emission_probs,
                                       value=tfd.Beta(1.1 + stats.sum_x, 1.1 + stats.sum_1mx).mode())

    @property
    def unconstrained_params(self):
        """Helper property to get a PyTree of unconstrained parameters."""
        return (
            tfb.SoftmaxCentered().inverse(self._initial_probabilities.value),
            tfb.SoftmaxCentered().inverse(self._transition_matrix.value),
            tfb.Sigmoid().inverse(self._emission_probs.value),
        )
    ...
    def unconstrained_params(self, unconstrained_params):
        ...
        self._emission_probs = replace(self._emission_probs, value=tfb.Sigmoid().forward(unconstrained_params[2]))
