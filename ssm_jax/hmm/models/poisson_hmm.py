...
class PoissonHMM(BaseHMM):
    ...
    def __init__(self, initial_probabilities, transition_matrix, emission_rates):
        ...
        self._emission_rates = Parameter(emission_rates)
    ...
    def emission_distribution(self, state):
        return tfd.Independent(tfd.Poisson(rate=self._emission_rates.value[state]), reinterpreted_batch_ndims=1)

    @property
    def emission_rates(self):
        return self._emission_rates.value
    ...
    def m_step(self, batch_emissions, batch_posteriors, **kwargs):
        ...
        self._emission_rates = replace(self._emission_rates,
                                       value=tfd.Gamma(1.1 + stats.sum_x, 1.1 + stats.sum_w).mode())
    ...
    def unconstrained_params(self):
        ...
        return (
            ...
            tfb.Softplus().inverse(self._emission_rates.value),
        )
    ...
    def unconstrained_params(self, unconstrained_params):
        ...
        self._emission_rates = replace(self._emission_rates, value=tfb.Softplus().forward(unconstrained_params[2]))
